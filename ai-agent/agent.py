...
class OutboundCaller(Agent):
    ...
    async def on_enter(self):
        ...
        await self.session.generate_reply(
            instructions=(
                "You just called the patient on behalf of a dental clinic. "
                "They just picked up the phone. Greet them warmly, introduce yourself, "
                "and let them know you're calling to confirm their upcoming appointment."
            )
        )
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        # Handle PowerShell stripped quotes (e.g. {phone_number: +91..., transfer_to: ...})
    ...
    # on_enter() on the agent handles the initial greeting automatically
